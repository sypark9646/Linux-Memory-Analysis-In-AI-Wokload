...
def save_graph(df, output_img_name, x_name, y_name, x_label, y_label):
  ...
  ax.legend(loc='best')
  plt.savefig(f'{output_img_name}_rankingLFU_accessCounts_linearscale.png') # save linear scale graph
  
  ax.set_xscale('log')
  ax.set_yscale('log')
  plt.savefig(f'{output_img_name}_rankingLFU_accessCounts_logscale.png') #save log scale graph
...
def main(read_file_name):
  ...
  save_graph(df, output_img_name, 'rank', 'access_number', 'LFU rank', 'access count')
  df['access_cum'] = df['access_number'].cumsum()
  df['access_cum_perc'] = 100 * df['access_cum']/df['access_number'].sum()
  save_graph(df, f"{output_img_name}_accumulated", 'rank', 'access_cum_perc', 'LFU rank', 'accumulative access count\(\%\)')
...
if __name__ == "__main__":
  parser = argparse.ArgumentParser(description = 'draw x-ranking(LFU) y-accessCount graph linear scale')
